...
**Critical finding**: oh-my-opencode uses "Sustainable Use License" - NOT permissive open source.
- Internal/non-commercial use only
- Cannot distribute commercially
- This eliminates Option A entirely

### My Conclusion: Option C (Hybrid, MIT-only)

**Why not A (use directly)**:
- ~~We want unique interaction modes~~
- **License prohibits commercial distribution** - non-starter
...
**Why C (hybrid, MIT sources only)**:
- claude-code-mcp is MIT ✅ - adopt patterns freely
- OpenCode core is MIT ✅ - use HTTP API directly
- General concepts (async, hooks) are not copyrightable
- Build our interaction modes as novel contribution
- Legally clean implementation path

---

## Patterns We're Adopting

### From claude-code-mcp (MIT ✅):
...
1. Async subprocess execution (standard Unix pattern)
2. Lifecycle hooks (standard middleware pattern)
3. Task completion enforcement (standard quality gate)
4. Model routing by task type (standard orchestration)
